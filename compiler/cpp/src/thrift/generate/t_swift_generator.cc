...
public:
    ...
    string type_name(t_type *ttype, bool is_optional = false, bool is_forced = false, bool is_outside_scope = false);
    ...
    string sanitize_identifier(const string &identifier);

    void populate_reserved_words();

    /** Swift 3 specific */
    string enum_case_name(t_enum_value *tenum_case);
...
private:
    ...
    string prepend_struct_namespacing(string type) {
        if (!struct_namespacing_) {
            return type;
        }
...
void t_swift_generator::init_generator() {
    ...
    // Put the generated entities in a so-called declaration file
    ...
    if (open_struct_namespace_) {
        indent_up();
        f_decl_ << "public struct " << module;
        block_open(f_decl_);
    }

    // Put the Swift extension in a separate file from the declarations
    string f_impl_name = name + "+Extensions.swift";
...
void t_swift_generator::close_generator() {
    ...
    if (struct_namespacing_ && open_struct_namespace_) {
        indent_down();
        block_close(f_decl_, true);
        open_struct_namespace_ = false;
    }
}
...
void t_swift_generator::generate_typedef(t_typedef *ttypedef) {
    f_decl_ << indent() << "public typealias " << ttypedef->get_symbolic()
            << " = " << type_name(ttypedef->get_type(), false, false, false) << endl;
    f_decl_ << endl;
}
...
string t_swift_generator::enum_case_name(t_enum_value *tenum_case) {
...
void t_swift_generator::generate_consts(vector<t_const *> consts) {
    ...
    for (c_iter = consts.begin(); c_iter != consts.end(); ++c_iter) {
        ...
        const_interface << "public let " << capitalize((*c_iter)->get_name()) << ": " << type_name(type, false, false,
                                                                                                   false) << " = ";
...
void t_swift_generator::generate_swift_struct(ostream &out,
    ...
    if (tstruct->is_union()) {
        ...
        for (m_iter = members.begin(); m_iter != members.end(); ++m_iter) {
            ...
            generate_docstring(out, doc);
            out << indent() << "case "
                << sanitize_identifier((*m_iter)->get_name()) << "(val: "
                << type_name((*m_iter)->get_type(), false, false, false) << ")" << endl;
        }
    } else {
        // Normal structs
        const string &access_modifier = function_access_modifier(is_private);
        out << indent() << access_modifier << " final class " << tstruct->get_name();
...
void t_swift_generator::generate_swift_struct_init(ostream &out,
                                                   ...
                                                   bool all,
                                                   bool is_private) {
    indent(out) << function_access_modifier(is_private) << " init(";
    ...
    for (m_iter = members.begin(); m_iter != members.end();) {
        if (all || !field_is_optional(*m_iter)) {
            ...
            out << sanitize_identifier((*m_iter)->get_name()) << ": "
                << sanitize_identifier(type_name((*m_iter)->get_type(), field_is_optional(*m_iter), false, false));
        }
        ++m_iter;
    }
    out << ")";
    ...
    for (m_iter = members.begin(); m_iter != members.end(); ++m_iter) {
        ...
        if (should_set) {
            out << indent() << "self." << sanitize_identifier((*m_iter)->get_name()) << " = "
                << sanitize_identifier((*m_iter)->get_name()) << endl;
        }
    }
...
void t_swift_generator::generate_swift_struct_hashable_extension(ostream &out,
                                                                 t_struct *tstruct,
                                                                 bool is_private) {
    indent(out) << "extension " << prepend_struct_namespacing(tstruct->get_name()) << ": Hashable";
    block_open(out);
    out << endl;
    indent(out) << function_access_modifier(is_private) << " func hash(into hasher: inout Hasher)";
    block_open(out);
    ...
    if (!members.empty()) {
        if (!tstruct->is_union()) {
            for (m_iter = members.begin(); m_iter != members.end(); ++m_iter) {
                ...
                indent(out) << "hasher.combine(" << sanitize_identifier(tfield->get_name()) << ")" << endl;
            }
        } else {
            indent(out) << "switch self {" << endl;
            for (m_iter = members.begin(); m_iter != members.end(); m_iter++) {
                t_field *tfield = *m_iter;
                indent(out) << "case let ." << tfield->get_name() << "(value): hasher.combine(value)" << endl;
            }
...
void t_swift_generator::generate_swift_struct_equatable_extension(ostream &out,
                                                                  t_struct *tstruct,
                                                                  bool is_private) {
    string namespaced_type_name = type_name(tstruct, false, false, true);
    indent(out) << function_access_modifier(is_private) << " func ==(lhs: " << namespaced_type_name << ", rhs: "
                << namespaced_type_name << ") -> Bool";
    block_open(out);
    indent(out) << "return";
    ...
    if (members.size()) {
        if (!tstruct->is_union()) {
            ...
            for (m_iter = members.begin(); m_iter != members.end();) {
                t_field *tfield = *m_iter;
                indent(out) << "(lhs." << sanitize_identifier(tfield->get_name())
                            << " == rhs." << sanitize_identifier(tfield->get_name())
                            << ")"; // swift 2 ?== operator not in 3?
...
void t_swift_generator::generate_swift_struct_thrift_extension(ostream &out,
    ...
    indent(out) << "extension " << prepend_struct_namespacing(tstruct->get_name()) << ": TStruct";
    block_open(out);
    out << endl;

    // generate fieldID's dictionary
    const string &access_modifier = function_access_modifier(is_private);
    out << indent() << access_modifier << " static var fieldIds: [String: Int32]";
    block_open(out);
    out << indent() << "return [";
    ...
    block_close(out);
    out << endl;
    out << indent() << access_modifier << " static var structName: String { return \""
...
void t_swift_generator::generate_swift_union_reader(ostream &out, t_struct *tstruct) {
    indent(out) << "public static func read(from sourceProtocol: TProtocol) throws -> "
                << prepend_struct_namespacing(tstruct->get_name());
    block_open(out);
    indent(out) << "_ = try sourceProtocol.readStructBegin()" << endl;
    ...
    block_open(out);
    out << endl;
    indent(out) << "let (_, fieldType, fieldID) = try sourceProtocol.readFieldBegin()" << endl << endl;
    ...
    for (f_iter = fields.begin(); f_iter != fields.end(); ++f_iter) {
        ...
        indent(out) << padding << "ret = " << tstruct->get_name() << "."
                    ...
                    << type_name((*f_iter)->get_type(), false, false, true)
                    << ".read(from: sourceProtocol))" << endl;
    }

    indent(out) << "case let (_, unknownType):  try sourceProtocol.skip(type: unknownType)" << endl;

    block_close(out);
    indent(out) << "try sourceProtocol.readFieldEnd()" << endl;

    block_close(out);
    out << endl;

    indent(out) << "try sourceProtocol.readStructEnd()" << endl;
...
void t_swift_generator::generate_swift_struct_reader(ostream &out,
    ...
    for (f_iter = fields.begin(); f_iter != fields.end(); ++f_iter) {
        ...
        indent(out) << "var " << sanitize_identifier((*f_iter)->get_name()) << ": "
                    << type_name((*f_iter)->get_type(), optional, !optional, true) << endl;
    }
    ...
    for (f_iter = fields.begin(); f_iter != fields.end(); ++f_iter) {
        ...
        out << padding << sanitize_identifier((*f_iter)->get_name()) << " = try "
            << type_name((*f_iter)->get_type(), false, false, true) << ".read(from: sourceProtocol)" << endl;
    }
    ...
    if (struct_has_required_fields(tstruct)) {
        ...
        for (f_iter = fields.begin(); f_iter != fields.end(); ++f_iter) {
            ...
            indent(out) << "try sourceProtocol.validateValue(" << sanitize_identifier((*f_iter)->get_name()) << ", "
    ...
    indent(out) << "return " << prepend_struct_namespacing(tstruct->get_name()) << "(";
    for (f_iter = fields.begin(); f_iter != fields.end();) {
        out << sanitize_identifier((*f_iter)->get_name()) << ": " << sanitize_identifier((*f_iter)->get_name());
...
void t_swift_generator::generate_swift_struct_printable_extension(ostream &out, t_struct *tstruct) {
    ...
    indent(out) << "extension " << prepend_struct_namespacing(tstruct->get_name()) << ": "
    ...
    if (!tstruct->is_union()) {
        ...
        for (f_iter = fields.begin(); f_iter != fields.end();) {
            indent(out) << "desc += \"" << (*f_iter)->get_name()
                        << "=\\(String(describing: self." << sanitize_identifier((*f_iter)->get_name()) << "))";
            if (++f_iter != fields.end()) {
                out << ",";
            }
            out << "\"" << endl;
...
void t_swift_generator::generate_swift_service_client_send_function_implementation(ostream &out,
    ...
    string proto = needs_protocol ? "on outProtocol" : "";
    // Open function
...
void t_swift_generator::generate_swift_service_client_recv_function_implementation(ostream &out,
    ...
    out << ") throws";
    if (!tfunction->get_returntype()->is_void()) {
        out << " -> " << type_name(tfunction->get_returntype(), false, false, false);
    }
    ...
    string return_type_name = type_name(tfunction->get_returntype(), false, false, false);
...
void t_swift_generator::generate_swift_service_client_send_async_function_invocation(ostream &out,
    ...
    indent(out) << "try send_" << tfunction->get_name() << "(on: sourceProtocol";
...
void t_swift_generator::generate_swift_service_client_implementation(ostream &out,
    ...
    indent(out) << "extension " << prepend_struct_namespacing(name) << ": " << prepend_struct_namespacing(
            tservice->get_name());
    block_open(out);
    out << endl;
...
void t_swift_generator::generate_swift_service_client_async_implementation(ostream &out, t_service *tservice) {
    ...
    indent(out) << "extension " << prepend_struct_namespacing(name) << ": " << protocol_name;
    block_open(out);
    out << endl;
    ...
    for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
        ...
        out << indent() << "let transport   = factory.newTransport()" << endl
            << indent() << "let sourceProtocol = Protocol(on: transport)" << endl
            << endl;
        ...
        if (!is_oneway) {
            ...
            out << "try self.recv_" << (*f_iter)->get_name() << "(on: sourceProtocol)" << endl;
...
void t_swift_generator::generate_swift_service_server_implementation(ostream &out,
    ...
    indent(out) << "extension " << prepend_struct_namespacing(name) << ": TProcessor";
    block_open(out);
    out << endl;
    ...
    for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
        ...
        if (!tfunction->is_oneway()) {
            ...
            if (!tfunction->is_oneway()) {
                out << indent() << "try outProtocol.writeMessageBegin(name: \"" << tfunction->get_name()
                    << "\", type: .reply, sequenceID: sequenceID)" << endl
 ...
 */
string t_swift_generator::type_name(t_type *ttype, bool is_optional, bool is_forced, bool is_outside_scope) {
    string result = "";
    ...
    if (ttype->is_base_type()) {
        ...
        result += "TMap<" + type_name(map->get_key_type(), false, false, true) + ", " +
                  type_name(map->get_val_type(), false, false, true) + ">";
    } else if (ttype->is_set()) {
        t_set *set = (t_set *) ttype;
        result += "TSet<" + type_name(set->get_elem_type(), false, false, true) + ">";
    } else if (ttype->is_list()) {
        t_list *list = (t_list *) ttype;
        result += "TList<" + type_name(list->get_elem_type(), false, false, true) + ">";
    } else {
        t_program *program = ttype->get_program();
        string is_diff = (program != program_) ? "diff" : "same";

        if ((module_namespacing_ || struct_namespacing_) && (program != program_ || is_outside_scope)) {
...
string t_swift_generator::base_type_name(t_base_type *type) {
    ...
    switch (tbase) {
        case t_base_type::TYPE_VOID:
            return "Void";
        case t_base_type::TYPE_STRING:
            return type->is_binary() ? "Data" : "String";
        case t_base_type::TYPE_BOOL:
            return "Bool";
        case t_base_type::TYPE_I8:
            return "Int8";
...
void t_swift_generator::render_const_value(ostream &out,
    ...
    if (type->is_base_type()) {
        ...
        switch (tbase) {
            ...
            case t_base_type::TYPE_I32:
            case t_base_type::TYPE_I64:
                out << type_name(type, false, false, false) << "(" << value->get_integer() << ")";
                break;
            case t_base_type::TYPE_DOUBLE:
                out << type_name(type, false, false, false) << "(";
...
string t_swift_generator::declare_property(t_field *tfield, bool is_private) {
    ...
    render << function_access_modifier(is_private) << " var " << sanitize_identifier(tfield->get_name());
    render << ": " << type_name(tfield->get_type(), field_is_optional(tfield), false, false);

    return render.str();
}

/**
 * Generates a function visibility string.
 *
 * @param is_private
 * @return
 */
string t_swift_generator::function_access_modifier(bool is_private) const {
    return is_private ? "fileprivate" : "public";;
}
...
string t_swift_generator::function_signature(t_function *tfunction) {
    ...
    if (!ttype->is_void()) {
        result += " -> " + type_name(ttype, false, false, false);
    }
...
void t_swift_generator::function_docstring(ostream &out, t_function *tfunction) {
    ...
    t_type *ttype = tfunction->get_returntype();
    if (!ttype->is_void()) {
        indent(out) << "/// - Returns: " << type_name(ttype, false, false, false) << endl;
    }
    ...
    vector<t_field *>::const_iterator x_iter;
    for (x_iter = xceptions.begin(); x_iter != xceptions.end(); ++x_iter) {
        out << type_name((*x_iter)->get_type(), false, false, false);
...
void t_swift_generator::async_function_docstring(ostream &out, t_function *tfunction) {
    ...
    // completion
    indent(out) << "///   - completion: TAsyncResult<" << type_name(tfunction->get_returntype(), false, false, false)
...
string t_swift_generator::async_function_signature(t_function *tfunction) {
    ...
    response_string += (ttype->is_void()) ? "Void" : type_name(ttype, false, false, false);
...
string t_swift_generator::argument_list(t_struct *tstruct, string protocol_name, bool is_internal) {
    ...
    for (f_iter = fields.begin(); f_iter != fields.end();) {
        ...
        result += arg->get_name() + ": " + type_name(arg->get_type(), field_is_optional(arg), false, false);
...
void t_swift_generator::populate_reserved_words() {
    ...
    swift_reserved_words_.insert("description");
    ...
    swift_reserved_words_.insert("read");
...
string t_swift_generator::sanitize_identifier(const string &identifier) {
    ...
    if (reserved_words_prefix_.empty()) {
        return "`" + identifier + "`";
    }

    return reserved_words_prefix_ + capitalize(identifier);
}
